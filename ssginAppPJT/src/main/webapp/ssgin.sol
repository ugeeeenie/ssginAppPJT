pragma solidity ^0.4.11;
 contract Ssgin {
    ...
    /* mapping 타입은 연관배열과 유사하다. 
     ...
     function joinPossibleCheck(bytes32 userId) constant returns (string) {
        if(existID(userId)){ // id가 존재한다면
            if(ssginAuth[userId].flag){ // 사용중인 id라면(flag가 true)
    ...
    function memberJoin(bytes32 userId, bytes32 userPwd) {
        User user;
        user.flag = true;
        user.id = userId;
        user.pwd = userPwd;
     ...
     function leavePossibleCheck(bytes32 userId) constant returns (string) {
        if(existID(userId)) { // id가 존재한다면
             if(ssginAuth[userId].flag) { // 사용중인 id라면(flag가 true)
     ...
     function memberLeave(bytes32 userId) {
