pragma solidity ^0.4.11;
 contract Ssgin {
 	event logData(bytes32 logId,bytes32 logPwd,bool logFlag);
    ...
    function memberJoin2(bytes32 userId, bytes32 userPwd) {
        ...
        logData(user.id, user.pwd, user.flag);
    }
    ...
    function makeId(bytes32 userId) constant returns (bytes32) {
         bytes32 id = sha3(userId);
         return id;
     }
     
     // 스트링 암호를 bytes32 형태로 변환
    function makePwd(bytes32 userPwd) constant returns (bytes32) {
         bytes32 pwd = sha3(userPwd);
         return pwd;
     }
     function voterDetails(bytes32 userId) constant returns (bytes32, bytes32,bool) {
     	return (ssginAuth[userId].id, ssginAuth[userId].pwd,ssginAuth[userId].flag);  
	}
 }
